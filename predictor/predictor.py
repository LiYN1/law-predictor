...
class Predictor(object):
	...
	def predict(self, content):
		fact_cut = [self.cut.cut(x, text=True) for x in content]
		vec = self.tfidf.transform(fact_cut)

		results = []
		for i in range(len(content)):
			ans = {}
			ans['accusation'] = self.predict_accu(vec[i])
			ans['articles'] = self.predict_law(vec[i])
			ans['imprisonment'] = self.predict_time(vec[i])
			results.append(ans)
